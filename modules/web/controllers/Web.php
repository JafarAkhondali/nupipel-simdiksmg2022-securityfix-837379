...
class Web extends Front
{
    ...
    {
        parent::__construct();
        $this->load->model('dashboard/dashboard_front', 'dashboard_front');
    }
    ...
    public function home()
    {
        if (defined('IS_DEMO')) {
            ...
            // GET MASTER SEKOLAH LAT LNG 
            $get = $this->dashboard_front->latLngSekolah();
            $latLng = [];
            foreach ($get as $data) {
                array_push($latLng, [
                    $data->lintang, $data->bujur, $data->nama_satuan_pendidikan
                ]);
            }


            //GET KECAMATAN LIST DATA 
            $getList = $this->dashboard_front->satpen_kecamatan();
            $labels = [];
            $values = [];

            foreach ($getList as $data) {
                array_push($labels, $data->kecamatan);
                array_push($values, $data->jml);
            }



            $data  = [
                'lat' => json_encode($latLng),
